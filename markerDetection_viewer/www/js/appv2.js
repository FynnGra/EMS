...
//If images width and height is not next power of two (e.g. 512x256)
//then you have to use minFilter.
      ...
      if(markers[64]) {
        if (marker64 == false) {
          ...
          for (var i = 0; i < quantity; i++) {
            ...
            var specificText = new THREE.Mesh(new THREE.BoxGeometry(50 * size, 20 * size, 1), beaconTextArray[i]);
...
*/
  ...
  .directive('webcamrightGl', [function() {
    ...
    function create(glFrame) {
      ...
      rendererRight.setSize(window.innerWidth, window.innerHeight);
